...
static int16_t Motion_Limit_Forward_Speed(int16_t speed)
{
    ...
    if (speed > MOTOR_MAX_FORWARD_SPEED_MM_S) {
        return MOTOR_MAX_FORWARD_SPEED_MM_S;
    }
    return speed;
}

static int16_t Motion_Ramp_Target(int16_t current, int16_t requested)
{
    if (requested == 0) {
        return 0;
    }
    if (requested > (current + MOTOR_TARGET_RAMP_STEP_MM_S)) {
        return (int16_t)(current + MOTOR_TARGET_RAMP_STEP_MM_S);
    }
    if (requested < (current - MOTOR_TARGET_RAMP_STEP_MM_S)) {
        return (int16_t)(current - MOTOR_TARGET_RAMP_STEP_MM_S);
    }
...
static int Motion_Abs_Encoder_Delta(int delta)
{
...
{
    return MD310_ENCODER_CIRCLE_PULSES;
}
...
void Motion_Set_Speed(int16_t speed_m1, int16_t speed_m2)
{
    ...
    {
        requested[i] = Motion_Limit_Forward_Speed(requested[i]);
        int16_t previous = motor_data.speed_set[i];
        int16_t next = Motion_Ramp_Target(previous, requested[i]);

        if ((previous != 0) && (next == 0)) {
            PID_Clear_Motor(i);
        }
        motor_data.speed_set[i] = next;
...
void Wheel_Yaw_Calc(float yaw)
{
    ...
    if (speed_L1 > MOTION_COMMAND_LIMIT_MM_S)
        speed_L1 = MOTION_COMMAND_LIMIT_MM_S;
    if (speed_L1 < -MOTION_COMMAND_LIMIT_MM_S)
        speed_L1 = -MOTION_COMMAND_LIMIT_MM_S;
   
    if (speed_R1 > MOTION_COMMAND_LIMIT_MM_S)
        speed_R1 = MOTION_COMMAND_LIMIT_MM_S;
    if (speed_R1 < -MOTION_COMMAND_LIMIT_MM_S)
        speed_R1 = -MOTION_COMMAND_LIMIT_MM_S;
...
void Motion_Get_Speed(car_data_t *car)
{
    ...
    {
        int pulse_delta = Motion_Abs_Encoder_Delta(g_Encoder_All_Offset[i]);

        /* Reverse is forbidden, so speed feedback is encoder magnitude only. */
        speed_mm[i] = pulse_delta * 50.0f * circle_mm / circle_pulse;
    }

    car->Vx = (speed_mm[0] + speed_mm[1] ) / 2;
    car->Vy = -(speed_mm[0] - speed_mm[1] ) / 2;
    car->Vz = -(speed_mm[0] - speed_mm[1]) / 2.0f / robot_APB *
              MOTION_YAW_RATE_SCALE;
...
{
    uint16_t input_speed = speed * MOTION_PERCENT_SPEED_SCALE;
...
void wheel_State_YAW(uint8_t state, uint16_t speed, uint8_t adjust)
{
    ...
    case MOTION_SPIN_LEFT:
        ...
        wheel_Ctrl(0, 0, -speed * MOTION_SPIN_SPEED_MULTIPLIER);
        break;
...
//    speed_lr = -V_y;
    speed_lr = 0;
    speed_fb = V_x;
    speed_spin = (V_z / MOTION_YAW_RATE_SCALE) * robot_APB;
    ...
    if (speed_L1_setup > MOTION_COMMAND_LIMIT_MM_S)
        speed_L1_setup = MOTION_COMMAND_LIMIT_MM_S;
    if (speed_L1_setup < -MOTION_COMMAND_LIMIT_MM_S)
        speed_L1_setup = -MOTION_COMMAND_LIMIT_MM_S;
   
    if (speed_R1_setup > MOTION_COMMAND_LIMIT_MM_S)
        speed_R1_setup = MOTION_COMMAND_LIMIT_MM_S;
    if (speed_R1_setup < -MOTION_COMMAND_LIMIT_MM_S)
        speed_R1_setup = -MOTION_COMMAND_LIMIT_MM_S;
...
void Motion_Handle(void)
{
    ...
    if (g_start_ctrl)
    {
        ...
        for (uint8_t i = 0; i < MAX_MOTOR; i++)
        {
            ...
            overspeed[i] =
                ...
                (Motion_Abs_Encoder_Delta(g_Encoder_All_Offset[i]) >
                 MOTOR_MAX_PULSES_PER_20MS);

            if ((motor_data.speed_set[i] == 0) || overspeed[i] ||
                (motor_data.speed_pwm[i] < 0.0f)) {
                motor_data.speed_pwm[i] = 0.0f;
        ...
        /* The first count above the 20 ms limit invokes active braking. */
        if (overspeed[0]) {
            Motor_Brake_Left();
        }
        if (overspeed[1]) {
