...
static int16_t dir_err_last = 0;
static float dir_integral = 0;

void Dir_PID_Reset(void)
{
    dir_err_last = 0;
    dir_integral = 0;
}
...
//   }
    dir_integral += error;           // 更新积分项，并进行限幅 Update the integral term and limit it
    if (dir_integral > Integral_Max) dir_integral = Integral_Max;        //积分限幅 Integral limiting
    if (dir_integral < -Integral_Max) dir_integral = -Integral_Max;      //积分限幅 Integral limiting

    // 位置式 PID
    result = dir_kp * error + dir_ki * dir_integral +
        dir_kd * (error - dir_err_last);
  
	 dir_err_last = error;       // 更新积分项，并进行限幅 Update the integral term and limit it
...
{ 
	...
	{	
		if (mpu_dmp_get_data((float *)&pitch, (float *)&roll,
		    (float *)&yaw) == 0)
        { 
          //  printf("\r\npitch =%d\r\n", (int)pitch);
           // printf("\r\nroll =%d\r\n", (int)roll);
	...
	else
	{
		...
		MPU6050ReadGyro((short *)angle);
		MPU6050ReadAcc((short *)accel);
