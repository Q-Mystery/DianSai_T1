...
void State_Machine_SelectTask(int task)
{
	State_Machine_init();
	q1_first_flag = 0;
	q2_first_flag = 0;
	...
	if (task >= QUESTION_1 && task <= QUESTION_4) {
		State_Machine.Main_State = task;
	}
}
...
void Question_Task_1(void)
{
	...
	//计算航向偏差	Calculate heading deviation
		// Temporarily disable heading PID for straight-line motor direction testing.
