...
#define OLED_ADDRESS_3C  (0x78U)
#define OLED_ADDRESS_3D  (0x7AU)

/* Drive OLED lines strongly; release SDA only while sampling ACK. */
#undef OLED_SCL_Clr
#undef OLED_SCL_Set
#undef OLED_SDA_Clr
#undef OLED_SDA_Set
...
#define OLED_SDA_Release() DL_GPIO_disableOutput(OLED_PORT, OLED_SDA_PIN)

u8 OLED_GRAM[OLED_WIDTH][OLED_PAGE_COUNT]; // OLED显存缓冲区 OLED display buffer
static u8 g_oled_write_address = OLED_ADDRESS_3C;
...
static bool OLED_ProbeAddress(u8 write_address)
{
    bool acknowledged;

    I2C_Start();
    Send_Byte(write_address);
    acknowledged = OLED_ReadAck();
    I2C_Stop();
    return acknowledged;
}
...
{
    I2C_Start();
    Send_Byte(g_oled_write_address); // OLED device address
    I2C_WaitAck();
    if(mode) {
...
void OLED_Refresh(void)
{
    ...
    for(i = 0; i < 4; i++) { // 4页(32行) 4 pages (32 rows)
        ...
        I2C_Start();
        Send_Byte(g_oled_write_address); // OLED address
        I2C_WaitAck();
        Send_Byte(0x40);     // 数据模式 Data mode
        I2C_WaitAck();
...
void OLED_Init(void)
{
    ...
    /* Release SCL/SDA for ACK sampling and recover a bus left mid-transfer. */
    ...
    OLED_SCL_Set();
    OLED_RecoverBus();
    if (OLED_ProbeAddress(OLED_ADDRESS_3C)) {
        g_oled_write_address = OLED_ADDRESS_3C;
    } else if (OLED_ProbeAddress(OLED_ADDRESS_3D)) {
        g_oled_write_address = OLED_ADDRESS_3D;
    } else {
        g_oled_write_address = OLED_ADDRESS_3C;
    }

    /* SSD1306 128x32, internal charge pump, page-addressing mode. */
