...
{
    /* Kept only for API compatibility with the former I2C sensor board. */
    (void) addr;
    (void) dat;
}
...
uint8_t IRI2C_ReadByte(uint8_t addr)
{
    uint8_t channel;
    uint8_t data = 0;
    const uint32_t addressPins =
        GRAYSCALE_AD2_PIN | GRAYSCALE_AD1_PIN | GRAYSCALE_AD0_PIN;

    (void) addr;
    for (channel = 0; channel < 8U; channel++) {
        uint32_t selectedPins = 0;

        if ((channel & 0x04U) != 0U) selectedPins |= GRAYSCALE_AD2_PIN;
        if ((channel & 0x02U) != 0U) selectedPins |= GRAYSCALE_AD1_PIN;
        if ((channel & 0x01U) != 0U) selectedPins |= GRAYSCALE_AD0_PIN;

        DL_GPIO_clearPins(GRAYSCALE_PORT, addressPins);
        DL_GPIO_setPins(GRAYSCALE_PORT, selectedPins);
        delay_cycles(16);

        if ((DL_GPIO_readPins(GRAYSCALE_PORT, GRAYSCALE_OUT_PIN) &
             GRAYSCALE_OUT_PIN) != 0U) {
            data |= (uint8_t) (1U << (7U - channel));
        }
    }
